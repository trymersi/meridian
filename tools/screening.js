...
export async function getTopCandidates({ limit = 10 } = {}) {
  ...
  if (eligible.length > 0) {
    ...
    // Bundle holding filter — pools where bundlers hold too much are high-risk
    if (config.screening.maxBundlePct != null) {
      eligible.splice(0, eligible.length, ...eligible.filter((p) => {
        if (p.bundle_pct != null && p.bundle_pct > config.screening.maxBundlePct) {
          log("screening", `Bundle filter: dropped ${p.name} — bundle ${p.bundle_pct}% > max ${config.screening.maxBundlePct}%`);
          pushFilteredReason(filteredOut, p, `bundle ${p.bundle_pct}% > maxBundlePct ${config.screening.maxBundlePct}%`);
          return false;
        }
        return true;
      }));
    ...
    if (Object.keys(getBlockedDevs()).length > 0) {
      const before = eligible.length;
    ...
    // Dumper wallet check — if the creator/deployer is a known dumper ring member
    let dumperFlagged = 0;
    eligible.forEach((p) => {
      const wallets = [p.dev, p.base?.mint].filter(Boolean);
      if (hasDumper(wallets)) {
        p.dumper_flag = { reason: "creator/deployer is known dumper wallet" };
        dumperFlagged++;
        log("dumper", `Flagged ${p.name} — creator is known dumper`);
      }
    });
    if (dumperFlagged > 0) log("dumper", `Flagged ${dumperFlagged} pool(s) with known dumper wallets`);
  }
