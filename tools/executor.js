...
import { addDumperWallet, removeDumperWallet, listDumperWallets } from "../dumper-wallets.js";
...
const toolMap = {
  ...
  add_dumper_wallet: addDumperWallet,
  list_dumper_wallets: listDumperWallets,
  remove_dumper_wallet: removeDumperWallet,
  ...
  update_config: ({ changes, reason = "" }) => {
    ...
    const CONFIG_MAP = {
      ...
      opportunityCooldownMin: ["opportunity", "cooldownMin"],
  ...
  try {
    ...
    if (success) {
      if (name === "swap_token" && result.tx) {
        ...
        notifyDeploy({ pair: result.pool_name || args.pool_name || args.pool_address?.slice(0, 8), amountSol: args.amount_y ?? args.amount_sol ?? 0, position: result.position, tx: result.txs?.[0] ?? result.tx, priceRange: result.price_range, rangeCoverage: result.range_coverage, binStep: result.bin_step, baseFee: result.base_fee, strategy: args.strategy, volatility: result.volatility }).catch(() => {});
        ...
        notifyClose({
          ...
          feeActiveTvlRatio: result.fee_active_tvl_ratio ?? null,
        }).catch(() => {});
